...
public record ProductRequest(String name, Double price, Double discount, String currency, Boolean isFeature,
		@NotNull Long categoryId, Long departmentId, Long brandId, List<String> imageUrls) {

	public Product mapToprProduct() {
		Department department = Department.builder()
				.id(DepartmentId.builder().departmentId(departmentId).categoryId(categoryId).build()).build();
		BrandCategory brandCategory = BrandCategory.builder()
				.brandCategoryId(BrandCategoryId.builder().brandId(brandId).categoryId(categoryId).build())

				.category(Category.builder().id(categoryId).build()).build();
		ProductId id = ProductId.builder().categoryId(categoryId).build();
		return Product.builder().id(id).name(name).price(price).discount(discount).currency(currency)
				.isFeature(isFeature).brand(Brand.builder().id(brandId).build()).department(department).brandCategory(brandCategory)
